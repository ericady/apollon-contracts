...
export function createPairCreatedEvent(token0: Address, token1: Address, pair: Address, param3: BigInt): PairCreated {
...
export function createSwapOperationsInitializedEvent(
  ...
  let swapOperationsInitializedEvent = changetype<SwapOperationsInitialized>(newMockEvent());

  swapOperationsInitializedEvent.parameters = new Array();

  swapOperationsInitializedEvent.parameters.push(
    new ethereum.EventParam('borrowerOperations', ethereum.Value.fromAddress(borrowerOperations)),
  );
  swapOperationsInitializedEvent.parameters.push(
    new ethereum.EventParam('troveManager', ethereum.Value.fromAddress(troveManager)),
  );
  swapOperationsInitializedEvent.parameters.push(
    new ethereum.EventParam('priceFeed', ethereum.Value.fromAddress(priceFeed)),
  );
  swapOperationsInitializedEvent.parameters.push(
    new ethereum.EventParam('debtTokenManager', ethereum.Value.fromAddress(debtTokenManager)),
  );
