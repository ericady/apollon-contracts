...
async function main() {
  ...
  const DebtToken = await DebtTokenFactory.deploy(TroveManager.target, RedemptionOperations.target, BorrowerOperations.target, StabilityPoolManager.target, PriceFeed.target, "JUSD", "Jelly", "1", true);
  await DebtToken.waitForDeployment();


  await DebtTokenManager.fuck(await DebtToken.getAddress())
