...
async function main() {
  ...
  */
  ...
  const weth = new ethers.Contract(WETH, WETH_ABI.abi, deployerWallet);
  ...
  if (currentWethBalance.lt(weth_balance)) {
    const txDepositWeth = await weth.deposit({
      value: weth_balance.sub(currentWethBalance),
    });
  ...
  // Approve LUSD
  const lusd = new ethers.Contract(LUSD, ERC20.abi, deployerWallet);
  ...
  console.log(
    'Total gas: ',
    receipt1.gasUsed
      .add(depositWethReceipt.gasUsed)
      .add(approveWethReceipt.gasUsed)
      .add(approveLusdReceipt.gasUsed)
