...
contract('PoolManager - random liquidations/deposits, then check all depositors can withdraw', async accounts => {
  ...
  const performLiquidation = async (remainingDefaulters, liquidatedAccountsDict) => {
    ...
    console.log(
      `Liquidation. addr: ${th.squeezeAddr(
        randomDefaulter
      )} ICR: ${ICRPercent}% coll: ${liquidatedETH} debt: ${liquidatedLUSD} SP LUSD before: ${LUSDinPoolBefore} SP LUSD after: ${LUSDinPoolAfter} tx success: ${
  ...
  const performSPDeposit = async (depositorAccounts, currentDepositors, currentDepositorsDict) => {
    ...
    console.log(
      `SP deposit. addr: ${th.squeezeAddr(randomDepositor)} amount: ${randomLUSDAmount} tx success: ${
        depositTx.receipt.status
      } `
    );
  };
  ...
  describe('Stability Pool Withdrawals', async () => {
    ...
    it("Defaulters' Collateral in range [1, 1e8]. SP Deposits in range [100, 1e10]. ETH:USD = 100", async () => {
      ...
      await th.openTrove_allAccounts_randomETH_randomLUSD(
        defaulterCollMin,
        defaulterCollMax,
        defaulterAccounts,
        contracts,
