...
contract HintHelpers is LiquityBase, Ownable(msg.sender), CheckContract {
  ...
  //    diff = LiquityMath._getAbsoluteDifference(_CR, troveManager.getCurrentICR(hintAddress));
  ...
  //      uint currentCR = troveManager.getCurrentICR(currentAddress);
  //
  //      // check if abs(current - CR) > abs(closest - CR), and update closest if current is closer
  //      uint currentDiff = LiquityMath._getAbsoluteDifference(currentCR, _CR);
  //      if (currentDiff < diff) {
  //        diff = currentDiff;
