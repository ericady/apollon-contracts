...
//    address public lqtyStakingAddress;
    ...
    function openTrove(uint _maxFeePercentage, TokenAmount[] memory _colls, TokenAmount[] memory _debts, address _upperHint, address _lowerHint) override external payable {
        ...
        bool isRecoveryMode = contractsCache.storagePool.checkRecoveryMode(priceCache);
        _requireValidMaxFeePercentage(_maxFeePercentage, isRecoveryMode);

        DebtTokenAmount memory stableCoinAmount;
        (vars.debts, stableCoinAmount) = _getDebtTokenAmountsWithFetchedPrices(contractsCache.debtTokenManager, priceCache, _debts);
        ...
        if (!isRecoveryMode)
            borrowingFeesPaid = _addBorrowingFees(contractsCache.troveManager, vars.debts, stableCoinAmount, _maxFeePercentage);

        // adding gas compensation to the net debt
        stableCoinAmount.netDebt = stableCoinAmount.netDebt.add(STABLE_COIN_GAS_COMPENSATION);
        ...
        if (isRecoveryMode) {
            ...
            uint newTCR = _getNewTCRFromTroveChange(vars.compositeCollInStable, true, vars.compositeDebtInStable, true);  // bools: coll increase, debt increase
        ...
        sortedTroves.insert(priceCache, msg.sender, vars.NICR, _upperHint, _lowerHint);
        ...
        for (uint i = 0; i < vars.colls.length; i++) {
            ...
            _poolAddColl(contractsCache.storagePool, collTokenAmount.tokenAddress, collTokenAmount.coll, PoolType.Active);
        }
        for (uint i = 0; i < vars.debts.length; i++) {
            DebtTokenAmount memory debtTokenAmount = vars.debts[i];
            _withdrawalDebt(contractsCache.storagePool, msg.sender, debtTokenAmount.debtToken, debtTokenAmount.netDebt, debtTokenAmount.netDebt.sub(debtTokenAmount.borrowingFee));
        }

        // Move the stable coin gas compensation to the Gas Pool
        contractsCache.storagePool.addValue(address(stableCoinAmount.debtToken), false, PoolType.GasCompensation, STABLE_COIN_GAS_COMPENSATION);
        stableCoinAmount.debtToken.mint(address(contractsCache.storagePool), STABLE_COIN_GAS_COMPENSATION);
...
//        collSurplusPool.claimColl(msg.sender);
    ...
    function _addBorrowingFees(ITroveManager _troveManager, DebtTokenAmount[] memory _debts, DebtTokenAmount memory _stableCoinAmount, uint _maxFeePercentage) internal returns (uint borrowingFee) {
...
//    }
    ...
    function _poolAddColl(IStoragePool _pool, address _collAddress, uint _amount, PoolType _poolType) internal {
        _pool.addValue(_collAddress, true, _poolType, _amount);
        IERC20(_collAddress).transferFrom(msg.sender, address(_pool), _amount);
    }

    function _withdrawalDebt(IStoragePool _storagePool, address _borrower, IDebtToken _debtToken, uint _netDebtIncrease, uint _mintAmount) internal {
...
//    }
    ...
    function _getDebtTokenAmountsWithFetchedPrices(IDebtTokenManager _dTokenManager, PriceCache memory _priceCache, TokenAmount[] memory _debts) internal returns (DebtTokenAmount[] memory debtTokenAmounts, DebtTokenAmount memory stableCoinEntry) {
    ...
    function _getCollTokenAmountsWithFetchedPrices(PriceCache memory _priceCache, TokenAmount[] memory _colls) internal returns (CollTokenAmount[] memory collTokenAmounts) {
    ...
    function _requireNotInRecoveryMode(PriceCache memory _priceCache) internal {
        require(!storagePool.checkRecoveryMode(_priceCache), "BorrowerOps: Operation not permitted during Recovery Mode");
    }
...
//        }
    ...
    function _requireValidLUSDRepayment(uint _currentDebt, uint _debtRepayment) internal pure {
        require(_debtRepayment <= _currentDebt.sub(STABLE_COIN_GAS_COMPENSATION), "BorrowerOps: Amount repaid must not be larger than the Trove's debt");
    }
...
//        return newTCR;
    ...
    function _getCompositeColl(CollTokenAmount[] memory _colls) internal pure returns (uint collInStable) {
        for (uint i = 0; i < _colls.length; i++) {
