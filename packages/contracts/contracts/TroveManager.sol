...
contract TroveManager is LiquityBase, Ownable, CheckContract, ITroveManager {
    ...
    **/
    ...
    function _liquidateNormalMode(IStoragePool _storagePool, PriceCache memory _priceCache, address _borrower, uint troveDebtInStable, RemainingStability[] memory remainingStabilities)
        internal
        returns (RAmount[] memory troveAmountsIncludingRewards)
    {
        ...
        _getOffsetAndRedistributionVals(_priceCache, troveDebtInStable, troveAmountsIncludingRewards, remainingStabilities);
        ...
        */
        ...
        for (uint i = 0; i < troveAmountsIncludingRewards.length; i++) {
            ...
            if (remainingStability.remaining > 0) {
                debtToOffset = LiquityMath._min(rAmount.toLiquidate, remainingStability.remaining);
                remainingStability.debtToOffset.add(debtToOffset);
                remainingStability.remaining = remainingStability.remaining.sub(debtToOffset);

                uint offsetPercentage = debtToOffset.mul(remainingStability.stabilityPool.getDepositToken().getPrice(_priceCache)).div(troveDebtInStable);
                ...
                for (uint ii = 0; ii < troveAmountsIncludingRewards.length; ii++) {
                    ...
                    for (uint iii = 0; iii < remainingStability.collGained.length; iii++) {
                        if (remainingStability.collGained[iii].tokenAddress != rAmountB.tokenAddress) continue;
...
//    }
    ...
    */
    function batchLiquidateTroves(address[] memory _troveArray) public override {
        ...
        vars.recoveryModeAtStart = vars.storagePoolCached.checkRecoveryMode(vars.priceCache);
...
//            vars.tokensToRedistribute = _getTotalFromBatchLiquidate_RecoveryMode(poolCached, remainingStabilities, vars.priceCache, _troveArray);
            ...
            (vars.tokensToRedistribute, vars.totalStableCoinGasCompensation) = _getTotalsFromBatchLiquidate_NormalMode(
...
//    }
    ...
    (
        ...
        internal returns (CAmount[] memory tokensToRedistribute, uint gasCompensationInStable)
    {
        LocalVariables_LiquidationSequence memory vars;
        vars.gasCompensationInStable = 0;

        // todo cache tokenAddresses + collTokenAddresses requets... gas savings
        vars.tokensToRedistribute = new CAmount[](tokenAddresses.length + collTokenAddresses.length);
        for (vars.i = 0; vars.i < tokenAddresses.length; vars.i++) {
            vars.tokensToRedistribute[vars.i] = CAmount(tokenAddresses[vars.i], false, 0);
        }
        for (vars.i = 0; vars.i < collTokenAddresses.length; vars.i++) {
            vars.tokensToRedistribute[tokenAddresses.length + vars.i] = CAmount(collTokenAddresses[vars.i], true, 0);
        }

        for (vars.i = 0; vars.i < _troveArray.length; vars.i++) {
            vars.user = _troveArray[vars.i];
            (vars.ICR, vars.troveDebtInStable) = this.getCurrentICR(vars.user, _priceCache);
            ...
            vars.troveAmountsIncludingRewards = _liquidateNormalMode(_pool, _priceCache, vars.user, vars.troveDebtInStable, remainingStabilities);

            // adding up the token to redistribute
            for (vars.ib = 0; vars.ib < vars.troveAmountsIncludingRewards.length; vars.ib++) {
                RAmount memory rAmount = vars.troveAmountsIncludingRewards[vars.ib];
                if (rAmount.toRedistribute == 0) continue;

                for (vars.ic = 0; vars.ic < vars.tokensToRedistribute.length; vars.ic++) {
                    if (vars.tokensToRedistribute[vars.ic].tokenAddress != rAmount.tokenAddress) continue;

                    vars.tokensToRedistribute[vars.ic].amount = vars.tokensToRedistribute[vars.ic].amount.add(rAmount.toRedistribute);
                    break;
                }
            }
        }
    ...
    function _sendGasCompensation(IStoragePool _storagePool, address _liquidator, uint _stableCoinGasCompensation) internal {
    ...
    function _movePendingTroveRewardsToActivePool(IStoragePool _storagePool, RAmount[] memory _troveAmountsIncludingRewards) internal {
        for (uint i = 0; i < _troveAmountsIncludingRewards.length; i++) {
            RAmount memory rAmount = _troveAmountsIncludingRewards[i];
            _storagePool.transferBetweenTypes(rAmount.tokenAddress, rAmount.isColl, PoolType.Default, PoolType.Active, rAmount.amount);
        }
    }
...
//    }
    ...
    function getCurrentICR(address _borrower, PriceCache memory _priceCache) external override returns (uint ICR, uint currentDebtInStable) {
    ...
    function _getCurrentTroveAmounts(address _borrower, PriceCache memory _priceCache) internal returns (uint currentCollInStable, uint currentDebtInStable) {
...
//        return (rewardSnapshots[_borrower].ETH < L_ETH);
    ...
    function getEntireDebtAndColl(address _borrower) external override view returns (RAmount[] memory amounts) {
...
0,0,0
        ...
        for (uint i = 0; i < amounts.length; i++) {
            amounts[i].gasCompensation = _getCollGasCompensation(amounts[i].amount.add(amounts[i].pendingReward));
            amounts[i].toLiquidate = amounts[i].amount.add(amounts[i].pendingReward).sub(amounts[i].gasCompensation);
        }
    }
...
//            emit TotalStakesUpdated(tokenAddress, totalStakes); todo
    ...
    function _redistributeDebtAndColl(IStoragePool _storagePool, PriceCache memory _priceCache, CAmount[] memory toRedistribute) internal {
        /*
...
//            emit LTermsUpdated(tokenAddress, liquidatedTokens[tokenAddress]); todo
            ...
            _storagePool.transferBetweenTypes(tokenAddress, toRedistribute[i].isColl, PoolType.Active, PoolType.Default, toRedistribute[i].amount);
        }
    }
...
//            emit SystemSnapshotsUpdated(tokenAddress, totalStakesSnapshot[tokenAddress], totalCollateralSnapshots[tokenAddress]); todo
    ...
    */
    ...
    function _requireDebtTokenBalanceCoversRedemption(IDebtToken _debtToken, address _redeemer, uint _amount) internal view {
...
//        return Troves[_borrower].coll;
    ...
    function increaseTroveColl(address _borrower, CollTokenAmount[] memory _collTokenAmounts) external override {
        _requireCallerIsBorrowerOperations();
    ...
    function decreaseTroveColl(address _borrower, CollTokenAmount[] memory _collTokenAmounts) external override {
        _requireCallerIsBorrowerOperations();
