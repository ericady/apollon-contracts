...
contract TroveManager is LiquityBase, Ownable, CheckContract, ITroveManager {
            ...
            if (remainingStability.remaining > 0) {
                ...
                for (uint ii = 0; ii < troveAmountsIncludingRewards.length; ii++) {
                    ...
                    for (uint iii = 0; iii < remainingStability.collGained.length; iii++) {
                        if (remainingStability.collGained[iii].tokenAddress != rAmountB.tokenAddress)
                            continue;
    ...
    function batchLiquidateTroves(address[] memory _troveArray) public override {
        ...
        vars.remainingStabilities = vars.stabilityPoolManagerCached.getRemainingStability(
            collTokenAddresses
        );
        ...
        for (vars.i = 0; vars.i < _troveArray.length; vars.i++) {
            ...
            for (vars.ib = 0; vars.ib < vars.troveAmountsIncludingRewards.length; vars.ib++) {
                ...
                for (vars.ic = 0; vars.ic < vars.tokensToRedistribute.length; vars.ic++) {
                    if (vars.tokensToRedistribute[vars.ic].tokenAddress != rAmount.tokenAddress)
                        continue;
    ...
    function getNominalICR(
        ...
        (uint currentCollInStable, uint currentDebtInStable) = _getCurrentTroveAmounts(
            _borrower,
            _priceCache
        );
    ...
    function getEntireDebtAndColl(
        ...
        for (uint i = 0; i < amounts.length; i++) {
            ...
            amounts[i].toLiquidate = amounts[i].amount.add(amounts[i].pendingReward).sub(
                amounts[i].gasCompensation
            );
        }
    }
