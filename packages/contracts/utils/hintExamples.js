...
async function main() {
  ...
  const LQTYContracts = await dh.deployLQTYContractsHardhat(ARBITRARY_ADDRESS, ARBITRARY_ADDRESS, ARBITRARY_ADDRESS);
  ...
  await borrowerOperations.openTrove(maxFee, LUSDAmount, upperHint, lowerHint, {
    value: ETHColl,
  });
  ...
  numTrials = numTroves.mul(toBN('15'))(({ 0: approxHint } = await hintHelpers.getApproxHint(NICR, numTrials, 42)))(
    // Use the approximate hint to get the exact upper and lower hints from the deployed SortedTroves contract
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, LUSDRepayment, false, upperHint, lowerHint, {
    value: collIncrease,
  });
