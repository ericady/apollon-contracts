...
contract(
  ...
  async accounts => {
    ...
    before(async () => {
      ...
      coreContracts = await deploymentHelper.deployLUSDTokenTester(
        coreContracts
      );
      const LQTYContracts =
    ...
    describe('LUSDToken', async accounts => {
      ...
      it('sendToPool(): reverts when called by an account that is not StabilityPool', async () => {
        // Attempt call from alice
        try {
          const txAlice = await lusdToken.sendToPool(
            bob,
            activePool.address,
            100,
            { from: alice }
      ...
      it('returnFromPool(): reverts when called by an account that is not TroveManager nor StabilityPool', async () => {
        // Attempt call from alice
        try {
          const txAlice = await lusdToken.returnFromPool(
            activePool.address,
            bob,
            100,
            {
    ...
    describe('CommunityIssuance', async accounts => {
      it('sendLQTY(): reverts when caller is not the StabilityPool', async () => {
        const tx1 = communityIssuance.sendLQTY(alice, dec(100, 18), {
          from: alice,
        });
        const tx2 = communityIssuance.sendLQTY(bob, dec(100, 18), {
          from: alice,
        });
        const tx3 = communityIssuance.sendLQTY(
