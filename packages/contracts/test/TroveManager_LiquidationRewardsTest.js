...
contract("TroveManager - Redistribution reward calculations", async accounts => {
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    assert.isAtMost(
      th.getDifference(bob_LUSDDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)),
      1000
    );
  });
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    ...
    const C_pendingRewardsAfterL1 = th
      ...
      .div(B_coll.add(C_coll));
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingETHReward(bob), B_pendingRewardsAfterL1),
      1000000
    );
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingETHReward(carol), C_pendingRewardsAfterL1),
      1000000
    );
    ...
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingETHReward(carol), C_pendingRewardsAfterL2),
      1000000
    );
    assert.isAtMost(
