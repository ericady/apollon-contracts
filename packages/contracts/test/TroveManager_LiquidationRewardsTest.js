...
contract('TroveManager - Redistribution reward calculations', async accounts => {
  ...
  it('redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards', async () => {
    ...
    */
    ...
    assert.isAtMost(
      th.getDifference(
        alice_Coll,
        A_collAfterL1.add(A_collAfterL1.mul(th.applyLiquidationFee(D_coll)).div(A_collAfterL1.add(C_coll)))
      ),
      1000
    );
    assert.isAtMost(
  ...
  it('redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards', async () => {
    ...
    const alice_Coll = (await troveManager.Troves(alice))[1]
      .add(await troveManager.getPendingETHReward(alice))
      .toString();
    const bob_Coll = (await troveManager.Troves(bob))[1].add(await troveManager.getPendingETHReward(bob)).toString();
    const dennis_Coll = (await troveManager.Troves(dennis))[1]
      .add(await troveManager.getPendingETHReward(dennis))
      .toString();
    const erin_Coll = (await troveManager.Troves(erin))[1].add(await troveManager.getPendingETHReward(erin)).toString();
    ...
    const gainedETH = th.applyLiquidationFee(
      E_coll.add(
        th.applyLiquidationFee(
          D_coll.add(
            th.applyLiquidationFee(C_coll.add(th.applyLiquidationFee(B_coll.add(th.applyLiquidationFee(A_coll)))))
          )
        )
      )
    );
  ...
  it('redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt', async () => {
    ...
    await borrowerOperations.withdrawLUSD(th._100pct, await getNetBorrowingAmount(A_totalDebt), alice, alice, {
      from: alice,
    });
  ...
  it('redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.', async () => {
    ...
    */
    ...
    const B_collAfterL1 = B_coll.add(B_coll.mul(th.applyLiquidationFee(C_coll)).div(A_coll.add(B_coll))).sub(
      withdrawnColl
    );
    const expected_B_coll = B_collAfterL1.add(B_collAfterL1.mul(th.applyLiquidationFee(D_coll)).div(totalCollAfterL1));
    const expected_B_debt = B_totalDebt.add(B_coll.mul(C_totalDebt).div(A_coll.add(B_coll))).add(
    ...
    th.assertIsApproximatelyEqual(
      entireSystemColl,
      A_coll.add(B_coll).add(th.applyLiquidationFee(C_coll)).sub(withdrawnColl).add(th.applyLiquidationFee(D_coll))
    );
  ...
  it('redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). A, B, C withdraw. E Enters, Liq(E). Distributes correct rewards', async () => {
    ...
    const bob_Coll_1 = (await troveManager.Troves(bob))[1].add(await troveManager.getPendingETHReward(bob)).toString();
    ...
    */
    ...
    const bob_Coll_2 = (await troveManager.Troves(bob))[1].add(await troveManager.getPendingETHReward(bob)).toString();
  ...
  it('redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards', async () => {
    ...
    assert.isAtMost(
      th.getDifference(
        defaultColl,
        C_collAfterL3.sub(C_collAfterL1).add(D_collAfterL3.sub(D_collAfterL2)).add(E_collAfterL3.sub(E_coll))
      ),
      1000000
    );
  ...
  it('redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards', async () => {
    ...
    */
    ...
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingETHReward(dennis), D_pendingRewardsAfterL2),
      10000000
    );
