...
contract('BorrowerWrappers', async accounts => {
  ...
  it('claimSPRewardsAndRecycle(): only owner can call it', async () => {
    // Whale opens Trove
    await openTrove({
      extraLUSDAmount: toBN(dec(1850, 18)),
      ICR: toBN(dec(2, 18)),
      extraParams: { from: whale },
    });
    ...
    await openTrove({
      extraLUSDAmount: toBN(dec(150, 18)),
      extraParams: { from: alice },
    });
  ...
  it('claimSPRewardsAndRecycle():', async () => {
    ...
    // Defaulter Trove opened
    const { lusdAmount, netDebt, collateral } = await openTrove({
      ICR: toBN(dec(210, 16)),
      extraParams: { from: defaulter_1 },
    });
    ...
    const expectedCompoundedLUSDDeposit_A = toBN(dec(150, 18)).sub(
      expectedLUSDLoss_A
    );
    ...
    th.assertIsApproximatelyEqual(
      troveDebtAfter,
      troveDebtBefore.add(proportionalLUSD)
    );
    ...
    th.assertIsApproximatelyEqual(
      depositAfter,
      depositBefore.sub(expectedLUSDLoss_A).add(netDebtChange)
    );
    ...
    th.assertIsApproximatelyEqual(
      stakeAfter,
      stakeBefore.add(expectedLQTYGain_A)
    );
  ...
  it('claimStakingGainsAndRecycle(): only owner can call it', async () => {
    ...
    // Defaulter Trove opened
    const { lusdAmount, netDebt, totalDebt, collateral } = await openTrove({
      ICR: toBN(dec(210, 16)),
      extraParams: { from: defaulter_1 },
    });
  ...
  it('claimStakingGainsAndRecycle(): reverts if user has no trove', async () => {
    ...
    const expectedLUSDGain_A = borrowingFee
      .mul(toBN(dec(150, 18)))
  ...
  it('claimStakingGainsAndRecycle(): with only ETH gain', async () => {
    ...
    const expectedNewLUSDGain_A = newBorrowingFee
      .mul(toBN(dec(150, 18)))
      .div(toBN(dec(2000, 18)));
    ...
    th.assertIsApproximatelyEqual(
      lusdBalanceAfter,
      lusdBalanceBefore.add(expectedNewLUSDGain_A)
    );
    // check trove has increased debt by the ICR proportional amount to ETH gain
    th.assertIsApproximatelyEqual(
      troveDebtAfter,
  ...
  it('claimStakingGainsAndRecycle(): with only LUSD gain', async () => {
    ...
    th.assertIsApproximatelyEqual(
      depositAfter,
      depositBefore.add(expectedLUSDGain_A),
      10000
    );
    // check lqty balance remains the same
    th.assertIsApproximatelyEqual(lqtyBalanceBefore, lqtyBalanceAfter);
  ...
  it('claimStakingGainsAndRecycle(): with both ETH and LUSD gains', async () => {
    ...
    th.assertIsApproximatelyEqual(
      depositAfter,
      depositBefore.add(expectedTotalLUSD),
      10000
    );
    // check lqty balance remains the same
    th.assertIsApproximatelyEqual(lqtyBalanceBefore, lqtyBalanceAfter);
