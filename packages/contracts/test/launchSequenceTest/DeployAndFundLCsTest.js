...
contract('Deploying and funding One Year Lockup Contracts', async accounts => {
  ...
  describe('Funding LCs', async accounts => {
    it('LQTY transfer from LQTY deployer to their deployed LC increases the LQTY balance of the LC', async () => {
      ...
      await lqtyToken.transfer(LCAddress_A, LQTYEntitlement_A, {
        from: multisig,
      });
      await lqtyToken.transfer(LCAddress_B, LQTYEntitlement_B, {
        from: multisig,
      });
      await lqtyToken.transfer(LCAddress_C, LQTYEntitlement_C, {
        from: multisig,
      });
      await lqtyToken.transfer(LCAddress_D, LQTYEntitlement_D, {
        from: multisig,
      });
      await lqtyToken.transfer(LCAddress_E, LQTYEntitlement_E, {
        from: multisig,
      });
  ...
  describe('Withdrawal attempts on funded, inactive LCs immediately after funding', async accounts => {
    it("Beneficiary can't withdraw from their funded LC", async () => {
      ...
      // Multisig transfers LQTY to each LC
      await lqtyToken.transfer(LC_A.address, LQTYEntitlement_A, {
        from: multisig,
      });
      await lqtyToken.transfer(LC_B.address, LQTYEntitlement_B, {
        from: multisig,
      });
      await lqtyToken.transfer(LC_C.address, LQTYEntitlement_C, {
        from: multisig,
      });
      ...
      for (LC of LCs) {
        try {
          ...
          const withdrawalAttemptTx = await LC.withdrawLQTY({
            from: beneficiary,
          });
