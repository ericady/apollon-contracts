  ...
  async accounts => {
    ...
    describe('Stability Pool Withdrawal', async () => {
      ...
      it('withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation', async () => {
        ...
        assert.isAtMost(
          th.getDifference(
            (await lusdToken.balanceOf(alice)).toString(),
        ...
        assert.isAtMost(
          th.getDifference(
            (await lusdToken.balanceOf(bob)).toString(),
            '6666666666666666666666'
          ),
          10000
        );
        assert.isAtMost(
          th.getDifference(
            (await lusdToken.balanceOf(carol)).toString(),
            '6666666666666666666666'
          ),
          10000
        );
      ...
      it('withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations', async () => {
        ...
        assert.isAtMost(
          th.getDifference((await lusdToken.balanceOf(alice)).toString(), '0'),
          10000
        );
        assert.isAtMost(
          th.getDifference((await lusdToken.balanceOf(bob)).toString(), '0'),
          10000
        );
        assert.isAtMost(
          th.getDifference((await lusdToken.balanceOf(carol)).toString(), '0'),
          10000
        );
      ...
      */
      ...
      it('withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and ETH Gain', async () => {
        ...
        assert.isAtMost(
          th.getDifference((await lusdToken.balanceOf(dennis)).toString(), '0'),
          100000
        );
      ...
      */
      ...
      it('withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool', async () => {
        ...
        assert.isAtMost(
          th.getDifference(
            (await lusdToken.balanceOf(erin)).toString(),
      ...
      it('withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool', async () => {
        ...
        assert.isAtMost(
          th.getDifference((await lusdToken.balanceOf(erin)).toString(), '0'),
          100000
        );
        assert.isAtMost(
          th.getDifference((await lusdToken.balanceOf(flyn)).toString(), '0'),
          100000
        );
        assert.isAtMost(
          th.getDifference((await lusdToken.balanceOf(graham)).toString(), '0'),
          100000
        );
        assert.isAtMost(
          th.getDifference(
            (await lusdToken.balanceOf(harriet)).toString(),
            '0'
          ),
          100000
        );
    ...
    */
      ...
      it('withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation', async () => {
        ...
        const SPLUSDBalance_1 = await lusdToken.balanceOf(
        ...
        // Check SP LUSD balance is zero
        const SPLUSDBalance_2 = await lusdToken.balanceOf(
          stabilityPool.address
        );
        ...
        // Check SP LUSD balance is zero
        const SPLUSDBalance_3 = await lusdToken.balanceOf(
          stabilityPool.address
        );
      ...
      it('withdrawFromSP(): Large liquidated coll/debt, deposits and ETH price', async () => {
        ...
        const aliceLUSDBalDiff = aliceLUSDBalance
          .sub(aliceExpectedLUSDBalance)
          .abs();
      ...
      it('withdrawFromSP(): Small liquidated coll/debt, large deposits and ETH price', async () => {
        ...
        const aliceLUSDBalance = await lusdToken.balanceOf(alice);
        const aliceExpectedLUSDBalance = toBN(
        ...
        const bobLUSDBalance = await lusdToken.balanceOf(bob);
        const bobExpectedLUSDBalance = toBN(
          '99999999999999997500000000000000000000'
        );
        const bobLUSDBalDiff = bobLUSDBalance.sub(bobExpectedLUSDBalance).abs();
