...
contract('StabilityPool - Withdrawal of stability deposit - Reward calculations', async accounts => {
  ...
  describe('Stability Pool Withdrawal', async () => {
    ...
    it('withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(100000, 18)), whale, whale, {
        from: whale,
      ...
      const txB = await stabilityPool.withdrawETHGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: bob,
      });
      const txC = await stabilityPool.withdrawETHGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: carol,
      });
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(alice)).toString(), '6666666666666666666666'),
        10000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '6666666666666666666666'),
        10000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '6666666666666666666666'),
        10000
      );
    ...
    it('withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations', async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(alice)).toString(), '3333333333333333333333'),
        10000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '3333333333333333333333'),
        10000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '3333333333333333333333'),
        10000
      );
    ...
    it('withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing LUSD', async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(5000, 18)),
        defaulter_1,
        defaulter_1,
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(7000, 18)),
        defaulter_2,
        defaulter_2,
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(alice)).toString(), '6000000000000000000000'),
        10000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '6000000000000000000000'),
        10000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '6000000000000000000000'),
        10000
      );
    ...
    it('withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing LUSD', async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(6000, 18)),
        defaulter_2,
        defaulter_2,
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(alice)).toString(), '4000000000000000000000'),
        10000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '4000000000000000000000'),
        10000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '4000000000000000000000'),
        10000
      );
    ...
    it('withdrawETHGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations', async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '13333333333333333333333'),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '20000000000000000000000'),
        100000
      );
    ...
    it('withdrawETHGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three identical liquidations', async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(alice)).toString(), '5000000000000000000000'),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '10000000000000000000000'),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '15000000000000000000000'),
        100000
      );
    ...
    it('withdrawETHGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations', async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(1000000, 18)), whale, whale, {
        from: whale,
      ...
      */
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount('207000000000000000000000'),
        defaulter_1,
        defaulter_1,
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(5, 21)),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount('46700000000000000000000'),
        defaulter_3,
        defaulter_3,
      ...
      // ()
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(alice)).toString(), '901719380174061000000'),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '205592018679686000000000'),
        10000000000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '5906261940140100000000'),
        10000000000
      );
    ...
    it('withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and ETH Gain', async () => {
      ...
      const txD = await stabilityPool.withdrawETHGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: dennis,
      });
      ...
      console.log();
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(alice)).toString(), '1666666666666666666666'),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '1666666666666666666666'),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '1666666666666666666666'),
        100000
      );

      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(dennis)).toString(), '5000000000000000000000'),
        100000
      );
    ...
    it('withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct LUSD deposit and ETH Gain', async () => {
      ...
      */
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(25000, 18)),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(40000, 18)),
        defaulter_4,
        defaulter_4,
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(alice)).toString(), '17832817337461300000000'),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '5944272445820430000000'),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '4458204334365320000000'),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(dennis)).toString(), '11764705882352900000000'),
        100000000000
      );
    ...
    it('withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct LUSD deposit and ETH Gain', async () => {
      ...
      */
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(alice)).toString(), '1672240802675590000000'),
        10000000000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '2090301003344480000000'),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '1045150501672240000000'),
        100000000000
      );
    ...
    it('withdrawETHGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 LUSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct LUSD deposit and ETH Gain', async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(alice)).toString(), '666666666666666666666'),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '666666666666666666666'),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '2000000000000000000000'),
        100000
      );
    ...
    it('withdrawETHGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool', async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '5000000000000000000000'),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(dennis)).toString(), '5000000000000000000000'),
    ...
    it('withdrawETHGainToTrove(): Depositors withdraw correct compounded deposit after liquidation empties the pool', async () => {
      ...
      const txE = await stabilityPool.withdrawETHGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: erin,
      });
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), '8333333333333333333333'),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(dennis)).toString(), '16666666666666666666666'),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(erin)).toString(), '25000000000000000000000'),
        100000
      );
    ...
    it('withdrawETHGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool', async () => {
      ...
      const txF = await stabilityPool.withdrawETHGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: flyn,
      });
      const txG = await stabilityPool.withdrawETHGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: graham,
      });
      const txH = await stabilityPool.withdrawETHGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: harriet,
      });
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(harriet)).toString(), '0'),
        100000
      );
    ...
    it('withdrawETHGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount('9999999910000000000000'),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, 'ether') }
      );
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(9900, 18)),
        defaulter_2,
        defaulter_2,
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), '100000000000000000000'),
        100000
      );
    ...
    it('withdrawETHGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount('59400000000000000000000'),
        defaulter_2,
        defaulter_2,
      ...
      */
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), dec(100, 18)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), dec(200, 18)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(dennis)).toString(), dec(300, 18)),
        100000
      );
    ...
    it('withdrawETHGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(bob)).toString(), dec(1, 17)),
        100000
      );
    ...
    it('withdrawETHGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount('59999400000000000000000'),
        defaulter_2,
        defaulter_2,
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(carol)).toString(), dec(2, 17)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(dennis)).toString(), dec(3, 17)),
        100000
      );
    ...
    it("withdrawETHGainToTrove(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount('9999999999900000000000'),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, 'ether') }
      );
    ...
    */
    it('withdrawETHGainToTrove(): Several deposits of 10000 LUSD span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedLUSDDeposit(dennis)).toString(), dec(99999, 12)),
        100000
      );
    ...
    it('withdrawETHGainToTrove(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(24100, 18)),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(200, 'ether') }
      );
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(24300, 18)),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2, value: dec(200, 'ether') }
      );
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(24500, 18)),
        defaulter_3,
        defaulter_3,
        { from: defaulter_3, value: dec(200, 'ether') }
      );
    ...
    it('withdrawETHGainToTrove(): Large liquidated coll/debt, deposits and ETH price', async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveLUSDAmount(dec(1, 36)),
        defaulter_1,
        defaulter_1,
    ...
    it('withdrawETHGainToTrove(): Small liquidated coll/debt, large deposits and ETH price', async () => {
      ...
      const txAPromise = stabilityPool.withdrawETHGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: alice,
      });
      const txBPromise = stabilityPool.withdrawETHGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
        from: bob,
      });
