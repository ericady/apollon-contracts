...
import accountsList from '@liquity/contracts/hardhatAccountsList2k';
...
const hasOracles = (network: string): network is keyof typeof oracleAddresses => network in oracleAddresses;
...
const getLiveArtifact = (name: string): { abi: JsonFragment[]; bytecode: string } => require(`./live/${name}.json`);
...
extendEnvironment(env => {
  env.deployLiquity = async (deployer, useRealPriceFeed = false, wethAddress = undefined, overrides?: Overrides) => {
...
task('deploy', 'Deploys the contracts to the network')
  ...
  .setAction(async ({ channel, gasPrice, useRealPriceFeed, createUniswapPair }: DeployParams, env) => {
...
task('setStorageSlotIndex', 'Returns the index of the balanceOf storage slot in an ERC20')
  ...
  .setAction(
    async (
      { contractAddress, walletAddress, slotIndex = 0, value = '1000000000000000000000' }: StorageSlotParams,
      hre
    ) => {
      ...
      await hre.ethers.provider.send('hardhat_setStorageAt', [
        contractAddress,
        utils.hexStripZeros(
          utils.keccak256(utils.defaultAbiCoder.encode(['address', 'uint'], [walletAddress, slotIndex]))
        ),
